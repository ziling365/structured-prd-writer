...
1. 创作者在内容编辑页上传素材。
2. 系统判断上传是否成功；失败时展示上传失败原因，成功时创建素材记录，状态为待校验。
3. 系统执行格式校验；校验失败时状态变更为校验失败并记录原因，校验通过时状态变更为待审核。
4. 系统调用审核服务获取内容审核结果；审核通过后素材可用于发布，审核拒绝后素材不可用于发布，审核超时后进入待人工处理。
5. 创作者点击发布时，系统校验关联素材是否全部审核通过。
6. 若全部审核通过，进入内容发布流程；若存在未通过素材，阻止发布并展示不可发布素材列表。
